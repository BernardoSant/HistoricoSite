...
export const DashAbastecimentos = () => {
  ...
  const prepareChartData = (abastecimento) => {
    ...
    const valoresAgrupados = mesesAnos.map((mesAno) => { 
        const totalKmL = abastecimento.filter((a) => a.dataCadastro.slice(0, 7) === mesAno).reduce((acc, curr) => acc + (curr.kmDiferença / curr.totalAbastecido), 0);
        const totalDiferença = abastecimento.filter((a) => a.dataCadastro.slice(0, 7) === mesAno).reduce((acc, curr) => acc + curr.kmDiferença, 0);     
        const totalGasolina = abastecimento.filter((a) => a.dataCadastro.slice(0, 7) === mesAno).reduce((acc, curr) => acc + curr.totalAbastecido, 0);
